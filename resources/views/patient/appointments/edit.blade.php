...
<body>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <input id="appointmentDate" class="form-control" type="date"
                                            ...
                                            value="{{ \Carbon\Carbon::parse($appointment->appointment_datetime)->format('Y-m-d') }}">
                                    </div>
                                    ...
                                    <div class="form-group mb-4">
                                        ...
                                        <input type="hidden" id="appointmentTime" name="appointment_time"
                                            value="{{ \Carbon\Carbon::parse($appointment->appointment_datetime)->format('H:i') }}" required>
                                    </div>
                                </div>
                            </div>
                        </div>
                                        ...
                                        <select id="branch" class="form-control" name="branch_id" required>
                                            @foreach ($branches as $branch)
                                                <option value="{{ $branch->id }}"
                                                    {{ $branch->id == $appointment->branch_id ? 'selected' : '' }}>
                                                    {{ $branch->name }}
                                                </option>
                                            @endforeach
                                        </select>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', (event) => {
            ...
            const appointmentTime = "{{ \Carbon\Carbon::parse($appointment->appointment_datetime)->format('H:i') }}";
            const currentTimeSlot = document.querySelector(`.time-slot[data-time="${appointmentTime}"]`);
            if (currentTimeSlot) {
                currentTimeSlot.classList.add('active');
            }
        });
    </script>
    <!-- Global Vendor -->
